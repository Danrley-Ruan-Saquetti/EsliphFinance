...
describe('Arquivar categoria (e2e)', () => {
  ...
  it('PATCH /categories/:id/archive arquiva a categoria (RN034)', async () => {
    ...
    expect(response.body.category.archivedAt).not.toBeNull()
  })

  it('PATCH /categories/:id/archive rejeita categoria inexistente com 404', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/categories/${randomUUID()}/archive`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send()
  ...
  it('PATCH /categories/:id/archive rejeita requisição sem token de acesso com 401', async () => {
    const response = await request(app.getHttpServer()).patch(`/categories/${randomUUID()}/archive`).send()
