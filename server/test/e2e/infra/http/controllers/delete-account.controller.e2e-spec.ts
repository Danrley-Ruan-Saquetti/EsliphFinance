...
describe('Excluir conta (e2e)', () => {
  ...
  it('DELETE /accounts/:id exclui a conta sem vínculo (RN024)', async () => {
    ...
    const response = await request(app.getHttpServer())
      .delete(`/accounts/${created.body.account.id}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send()
  ...
  it('DELETE /accounts/:id exclui a conta arquivada (RN024, RN025)', async () => {
    const created = await request(app.getHttpServer())
      .post('/accounts')
      .set('Authorization', `Bearer ${accessToken}`)
  ...
  it('DELETE /accounts/:id rejeita conta inexistente com 404', async () => {
    const response = await request(app.getHttpServer()).delete(`/accounts/${randomUUID()}`).set('Authorization', `Bearer ${accessToken}`).send()

    expect(response.statusCode).toBe(404)
    expect(response.body.code).toBe('RESOURCE_NOT_FOUND')
  })

  it('DELETE /accounts/:id rejeita requisição sem token de acesso com 401', async () => {
    const response = await request(app.getHttpServer()).delete(`/accounts/${randomUUID()}`).send()
