...
describe('Criar grupo de ativo (e2e)', () => {
  ...
  it('POST /asset-groups cria o grupo de ativo', async () => {
    ...
    expect(response.body.assetGroup).toEqual(expect.objectContaining({ id: expect.any(String) as string, name: 'Cartões', type: 'CREDIT_CARD' }))
  })

  it('POST /asset-groups persiste o grupo de ativo vinculado ao usuário autenticado (RN010)', async () => {
    ...
    const [record] = await drizzle.db
      .select()
      .from(assetGroups)
      .where(eq(assetGroups.id, created.body.assetGroup.id as string))
  ...
  it('POST /asset-groups rejeita corpo inválido com 422', async () => {
    const response = await request(app.getHttpServer()).post('/asset-groups').set('Authorization', `Bearer ${accessToken}`).send({ name: '' })
  ...
  it('POST /asset-groups rejeita tipo fora do domínio permitido com 422 (RN015)', async () => {
    const response = await request(app.getHttpServer())
      .post('/asset-groups')
      .set('Authorization', `Bearer ${accessToken}`)
  ...
  it('POST /asset-groups rejeita requisição sem token de acesso com 401', async () => {
    const response = await request(app.getHttpServer()).post('/asset-groups').send({ name: 'Contas' })
