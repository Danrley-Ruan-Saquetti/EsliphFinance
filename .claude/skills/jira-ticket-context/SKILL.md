...
- **Existe em `docs/requirements/` com o mesmo número?** Se não, ou se o texto diverge do que a issue descreve, isso é material para a `business-analyst` decidir — não corrija o requisito você mesma, nem finja que bate.
...
Não é seu reescrever `docs/requirements/` nem `docs/open-decisions.md` — isso é sempre da `business-analyst`, mesmo quando a divergência veio à tona por aqui.
