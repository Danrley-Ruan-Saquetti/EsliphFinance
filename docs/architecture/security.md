...
| `POST /sessions` | `AuthenticateUserController` | Login (RN004) |
| `POST /sessions/refresh` | `RefreshSessionController` | Renovação sem credencial (RN006) |
...
## Isolamento dos registros por usuário (RN010, RN011)
...
Todo caso de uso que lê ou altera registro entra com teste de acesso cruzado entre dois usuários, citando RN010 e RN011 no nome, no unitário e no e2e.
...
| ----- | ------------- | ------------- |
| `HashGenerator`, `HashComparer` | `BcryptHasher` | bcrypt com 10 rounds; hash irreversível de senha (RNF006). Uma instância só atende às duas portas, via `useExisting` |
| `AccessTokenGenerator` | `JwtAccessTokenGenerator` | Assina HS256 com `JWT_SECRET`, expiração de `ACCESS_TOKEN_EXPIRES_IN_SECONDS` (RN005) |
...
O token de renovação **não é JWT**: é um segredo opaco, e o banco guarda apenas o SHA-256 dele. Quem vazar a tabela não consegue renovar sessão. É também o que permite invalidar o token na rotação (RN007) e no logout (RN008) — algo que um JWT autocontido não permitiria sem uma lista de revogados.
