...
| `DrizzleAccountsRepository` | `src/infra/database/drizzle/repositories/drizzle-accounts-repository.ts` | Todos os filtros em SQL; `innerJoin` com `account_groups` (é o que viabiliza filtrar por tipo); `leftJoin` com `transactions` na condição `account_id = accounts.id OR source_account_id = accounts.id OR destination_account_id = accounts.id` (SCRUM-53) + `groupBy(accounts.id)` para compor o **saldo** (RN021, RN050) — soma condicional (`sum(case when status = 'SETTLED' then ...)`) que resolve o sinal por `type` **e** por qual das três colunas bateu com a conta: `INCOME`/`EXPENSE` somam ou subtraem quando `account_id` é a conta; "TRANSFER" soma quando a conta é `destination_account_id` e subtrai quando é `source_account_id`. O resultado do `sql` agregado chega como `string` (Postgres devolve `numeric`/`bigint` fora de coluna tipada como texto), por isso `toDomain` faz `Number(balanceInCents)` antes de `Money.fromCents`; ordena por nome; `save` é um `update` por id, `delete` é um `delete` por id. |
| `InMemoryAccountsRepository` | `src/infra/database/in-memory/in-memory-accounts-repository.ts` | **Recebe o `InMemoryAccountGroupsRepository` e o `InMemoryTransactionsRepository` no construtor**, os dois obrigatórios (SCRUM-55 tornou o segundo obrigatório para honrar a mesma porta do `DrizzleAccountsRepository` sem exceção — todo spec do contexto precisa montar os três repositórios, mesmo quando não testa saldo). `calculateBalance` espelha a mesma regra de sinal do Drizzle: `affectsAccount` decide se a transação toca a conta por `accountId`, `sourceAccountId` ou `destinationAccountId`; `signedAmountInCents` soma para `destinationAccountId` e subtrai para `sourceAccountId` quando o tipo é "TRANSFER" (SCRUM-53). `save` substitui o item pelo índice do `id`. |
...
| RN021 | `DrizzleAccountsRepository.findManyByOwnerId` / `InMemoryAccountsRepository.calculateBalance` | Saldo = `initial_balance` + soma das _Transações_ "Efetivadas" da conta (RN050). Inclui receita/despesa lançadas na conta **e** os dois lados de uma [transferência](transaction.md) (crédito quando a conta é o destino, débito quando é a origem — RN044, SCRUM-53). Agregado na consulta, não em memória na entidade — é por isso que `AccountWithBalance.balance` vem do repositório. A operação inteira é atômica por construção (RNF008): a transferência é um único registro de _Transação_ com os dois `accountId`, então um único `insert` já garante os dois lados juntos, sem precisar coordenar duas escritas. |
| RN050 | `DrizzleAccountsRepository.findManyByOwnerId` (`sum` condicional por `status`) / `InMemoryAccountsRepository.calculateBalance` (filtro por `status`) | Só [`Transaction`](transaction.md) com `status = 'SETTLED'` entra na soma — vale igual para receita, despesa e os dois lados de uma transferência; "Prevista" nunca afeta o `balance` devolvido. O **saldo projetado** (a metade "Previstas" da regra) não tem consulta própria ainda — ver "Ainda não existe". |
...
| [Transações](transaction.md) | Transação → Conta | `CreateTransactionUseCase` e `CreateTransferUseCase` injetam `AccountsRepository` para checar dono e arquivamento antes de registrar o lançamento; `CreateTransferUseCase` também injeta `AccountGroupsRepository` para exigir grupo do tipo "Padrão" nos dois lados (RN045); `DrizzleAccountsRepository`/`InMemoryAccountsRepository` somam as _Transações_ "Efetivadas" no `balance` agregado da listagem, inclusive os dois lados de uma transferência (SCRUM-53, SCRUM-55) | RN010, RN011, RN021, RN025, RN044, RN045, RN050 |
